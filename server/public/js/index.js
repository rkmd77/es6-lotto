	...
	_createClass(Timer, [{
		...
		value: function countdown(end, update, handle) {
			...
			if (now - end) {
				...
				var px_m = 1000 * 60; //一分钟的毫秒值
				var px_s = 1000; //一秒的毫秒值
				var d = Math.floor(last_time / px_d); //剩余时间为多少天
				var h = Math.floor((last_time - d * px_d) / px_h); //剩余时间为多少小时
				var m = Math.floor((last_time - d * px_d - h * px_h) / px_m); //剩余时间为多少分钟
				var s = Math.floor((last_time - d * px_d - h * px_h - m * px_m) / px_s); //剩余时间为多少秒
				var r = [];
				if (d > 0) {
					r.push('<em>' + d + '</em>\u5929');
				}
